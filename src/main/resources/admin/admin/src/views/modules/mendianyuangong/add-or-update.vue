<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="部门" prop="bumen">
          <el-select v-model="ruleForm.bumen" placeholder="请选择部门">
            <el-option
                v-for="(item,index) in bumenOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="个人简历" prop="gerenjianli">
          <file-upload
          tip="点击上传个人简历"
          action="file/upload"
          :limit="1"
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="店主姓名" prop="dianzhuxingming">
          <el-input v-model="ruleForm.dianzhuxingming" 
              placeholder="店主姓名" clearable  :readonly="ro.dianzhuxingming"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="店主姓名" prop="dianzhuxingming">
              <el-input v-model="ruleForm.dianzhuxingming" 
                placeholder="店主姓名" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.dianzhuzhanghao = true;
          ...
          if(o=='dianzhuxingming'){
            this.ruleForm.dianzhuxingming = obj[o];
	    this.ro.dianzhuxingming = true;
            continue;
          }
        }
      }
      ...
      this.$http({
		...
		if(json.dianzhuxingming!=''&&json.dianzhuxingming){
              		this.ruleForm.dianzhuxingming = json.dianzhuxingming
		}
        } else {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `mendianyuangong/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.mendianyuangongCrossAddOrUpdateFlag = false;
                  this.parent.search();
